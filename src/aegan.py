...
class AEGAN(LightningModule):
    ...
    def __init__(
        ...
        self._init_dx()
        self._init_dz()
        self.validation_z = torch.randn(batch_size, latent_vector_size, 1, 1).to(device)


    def _init_generator(self):
        self.generator = Generator(latent_dim=self.latent_vector_size)
    ...
    def _init_encoder(self):
        self.encoder = Encoder(latent_dim=self.latent_vector_size, device=self.device)
    ...
    def _init_dz(self):
        self.discriminator_latent = DiscriminatorLatent(
            latent_dim=self.latent_vector_size,
            device=self.device,
            ).to(self.device)
    ...
    def train_step_generators(self, X):
        ...
        Z_recon_loss = self.criterion_recon_latent(Z_tilde, self.validation_z) * ALPHA_RECONSTRUCT_LATENT
    ...
    def train_step_discriminators(self, X):
        ...
        with torch.no_grad():
            X_hat = self.generator(self.validation_z)
            Z_hat = self.encoder(X)
            X_tilde = self.generator(Z_hat)
            Z_tilde = self.encoder(X_hat)
        ...
        X_hat_confidence = self.discriminator_image(X_hat)
        X_tilde_confidence = self.discriminator_image(X_tilde)
        Z_confidence = self.discriminator_latent(self.validation_z)
        Z_hat_confidence = self.discriminator_latent(Z_hat)
        Z_tilde_confidence = self.discriminator_latent(Z_tilde)
